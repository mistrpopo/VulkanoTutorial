...
fn main() {
	...
	let mut image = create_image(device.clone(), queue.clone());
	//clear_image(image.clone(), device.clone(), queue.clone());

	image = create_image(device.clone(), queue.clone());
...
fn simple_gpu_shader_compute(device: Arc<Device>, queue: Arc<Queue>) {
	...
	let shader = simple_shader::Shader::load(device.clone()).expect("failed to create shader module");
...
fn generate_mandelbrot(image: Arc<StorageImage<Format>>, device: Arc<Device>, queue: Arc<Queue>) {
	//create the compute pipeline from mandelbrot_shader mod
	...
	println!("Start executing command buffer");
	let finished = command_buffer.execute(queue.clone()).unwrap();
	finished.then_signal_fence_and_flush().unwrap().wait(None).unwrap();
	println!("Done executing command buffer");

	println!("Read buffer from GPU");
	let buffer_content = buf.read().unwrap();
